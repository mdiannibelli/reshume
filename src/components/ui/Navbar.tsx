      ...
      >
        <header className="flex justify-between lg:grid lg:grid-cols-3 items-center px-6 md:px-8 lg:px-4">
         ...
         <div className="flex items-center gap-x-4 justify-end lg:mx-auto col-span-1">
          ...
          <Link to="/login" className="hidden lg:block text-white bg-black hover:translate-y-[-2px] rounded-full px-4 xl:px-6 py-2 transition-all duration-300">{t("NAVBAR.LOGIN")}</Link>  
          <Link to="/generate-resume" className="bg-red-500 text-white font-medium rounded-full hover:translate-y-[-2px] px-4 md:px-6 py-2 transition-all duration-300">{t("NAVBAR.GENERATE_RESUME")}</Link>  
         </div>
        </header>
      </div>
    );
