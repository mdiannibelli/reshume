    ...
    <>
      ...
      <div className="relative z-50 -translate-y-20">
        ...
        >
          ...
          <Link
            to="/generate-resume"
            className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-red-500 md:px-6 px-4 py-1 text-lg md:text-xl font-medium text-white backdrop-blur-3xl"
          >
            {btnName}
          </Link>
        </motion.button>
        ...
        >
          {features.map((feature) => (
            <motion.li className="flex items-center gap-2" key={feature}>
              <RiCheckboxCircleFill className="text-lg text-red-500" />
              <span>{feature}</span>
            </motion.li>
          ))}
        </motion.ul>
