...
export const MenuItem = ({
    ...
    <div onMouseEnter={() => setActive(typeof item === 'string' ? item : '')} className="relative ">
      ...
      {active !== null && (
        ...
        >
          {active === item && (
            <div className="absolute top-[calc(100%-0.75rem)] left-12 transform -translate-x-1/2 pt-4">
              <motion.div
                transition={transition}
                layoutId="active"
...
export const Menu = ({
  ...
  return (
    <div
      ...
      className={`relative rounded-full border border-transparent bg-black shadow-input flex justify-center space-x-4 py-6 ${className}`}
    >
      {children}
    </div>
  );
